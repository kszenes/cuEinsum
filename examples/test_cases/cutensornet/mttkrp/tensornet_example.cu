...
int main()
{
   ...
   typedef float floatType;
   cudaDataType_t typeData = CUDA_R_32F;
   cutensornetComputeType_t typeCompute = CUTENSORNET_COMPUTE_TF32;
   #undef TENSOR
   #elif defined(DOUBLE)
   ...
   **********************/
   ...
   std::vector<int32_t> modesA{'i', 'j', 'k'};
   std::vector<int32_t> modesB{'j', 'a'};
   std::vector<int32_t> modesC{'k', 'a'};
   std::vector<int32_t> modesD{'i', 'a'};
   ...
   *******************************/
   ...
   // cutensornetContractionPath_t contractionPath;
   // HANDLE_ERROR( cutensornetContractionOptimizerInfoGetAttribute(
   //             handle,
   //             optimizerInfo,
   //             CUTENSORNET_CONTRACTION_OPTIMIZER_INFO_PATH,
   //             &contractionPath,
   //             sizeof(contractionPath)));
   // printf("Num contractions: %d\n", contractionPath.numContractions);
   // printf("\tContraction %d: %d and %d\n", 1, contractionPath.data[1].first, contractionPath.data[1].second);
   // for (int iter = 0; iter < contractionPath.numContractions; iter++) {
   //    printf("\tContraction %d: %d and %d\n", iter, contractionPath.data[iter].first, contractionPath.data[iter].second);
   // }
